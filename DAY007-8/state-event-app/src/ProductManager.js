...
const ProductManager = () => {
    ...
    const handleProduct = (e) => {
        // 리액트에서는 UI요소와 바인딩된 데이터소스의 값을 변경할 때
        // 데이터 소스가 변경되었는지를 자동으로 감지, 근데 참조타입을 그냥 복사하더나
        // 속성의 값을 그대로 변경하는 경우 메모리주소가 변경되지 않기 때문에 변경을 감지할 수 없음
        // 그래서 Deep Copy의 일종인 spread 연산자를 통해 원본 데이터 소스의 복사본을 만들어
        // 복사본의 새로운 메모리주소와 공간을 할당하여 객체가 변경되게 처리하여 데이터 변경 사실을
        // 데이터바인딩 엔진이 감지할 수 있도록 처리
